"""Leave-one-subject-out (LOSO) on WAUC — the leakage-clean WAUC evaluation.

WAUC's within-subject CV is block-design-confounded in the same way as STEW. The
loader (``wauc_load.py``) assigns *one* binary mental-workload label to every epoch
of a subject's session, and each subject contributes six sessions (2 mental-workload
x 3 physical-exertion cells). Random K-fold CV therefore trains and tests on epochs
cut from the *same* sessions, so a classifier can score by session identity ("which
of my six sessions is this window from") rather than by workload. The within-CV
kappa (≈0.65, not ceiling-saturated only because six harder-to-separate blocks leak
less catastrophically than STEW's two) is consequently a partial recording-identity
score, not a clean workload score.

LOSO removes the leakage structurally: pool every subject, hold one out for test,
train on the others. The held-out subject's six sessions are unseen at fit time, so
session recognition is impossible and the reported kappa measures genuine
cross-subject workload decoding. (LOSO is chosen over leave-one-session-out because
each WAUC session is single-MW-class, which makes a per-session test fold degenerate
for kappa; a held-out subject always contains both classes.)

Three conditions, matching the WAUC within-CV panel and the STEW LOSO format:
...
Two kappa figures per method: per-subject mean ± std over the held-out subjects
(degenerate single-class test subjects excluded and counted), and pooled global
kappa over every held-out prediction (the robust LOSO headline).

Output: ``results/loso_wauc.csv`` (one row per (method, subject, seed)).

Examples::

    PYTHONPATH=src .venv/bin/python scripts/run_loso_wauc.py
    PYTHONPATH=src .venv/bin/python scripts/run_loso_wauc.py --subjects 1,2,3,4 \\
        --output results/loso_wauc_smoke.csv
    PYTHONPATH=src .venv/bin/python scripts/run_loso_wauc.py --methods fbcsp,bandpower

Requires data/WAUC/ populated; see data/WAUC.README.md.
"""
...
from thesis.data.wauc_load import WAUC_CHANNEL_ROLES, WAUC_EEG_CHANNELS, load_wauc
...
def main(
    subjects: str = typer.Option(
        "all", help="Comma-separated WAUC subject IDs (1..48), or 'all'."
    ),
    ...
    ccb_seeds: str = typer.Option(
        "42", help="Comma-separated CCB seeds. Default single seed."
    ),
    train_cap: int = typer.Option(
        4000,
        help="Common per-fold training-pool cap applied IDENTICALLY to every method "
        "(0 = use all). WAUC pools ~34k epochs across 44 subjects; one class-stratified "
        "4k subsample (fixed split seed) keeps FBCSP and EEGNet tractable AND gives the "
        "fixed baselines, the CCB, and EEGNet the identical training pool "
        "(matched-conditions discipline; src/thesis/matched.py).",
    ),
    alpha: float = typer.Option(0.5, help="OPLB α (matches within-CV WAUC headline)."),
    ...
    output: Path = typer.Option(Path("results/loso_wauc.csv"), help="Output CSV path."),
) -> None:
    console = Console()
    ...
    wauc = load_wauc(subjects=subject_filter)
    if len(wauc) < 2:
        console.print("[red]✗ LOSO needs ≥2 subjects. Check data/WAUC/ (see data/WAUC.README.md).[/red]")
        raise typer.Exit(code=1)
    console.log(f"Loaded {len(wauc)} subjects → {len(wauc)} LOSO folds.")

    n_channels = wauc[0].X.shape[1]
    assert n_channels == len(WAUC_EEG_CHANNELS), f"unexpected channel count {n_channels}"

    # Matched-conditions folds: each split's train_idx is the common class-stratified
    # train_cap subsample (fixed split seed), shared identically by every method below.
    folds = list(matched_loso(wauc, cap=train_cap))
    rows: list[dict] = []
        ...
        for fold_data, split in folds:
            ...
            try:
                rec, yt, yp = _fixed_fold(
                    name, fold_data, split, channel_roles=WAUC_CHANNEL_ROLES
                )
                rows.append({"dataset": "WAUC", "method": name, "subject": sid, "seed": -1, **rec,
                ...
                rows.append({"dataset": "WAUC", "method": name, "subject": sid, "seed": -1,
        ...
        for seed in seed_list:
            ...
            for fold_data, split in folds:
                ...
                try:
                    ...
                    rows.append({"dataset": "WAUC", "method": "ccb", "subject": sid, "seed": seed,
    ...
    # ---- Summary ----
    console.rule("LOSO-on-WAUC — per-subject mean ± std κ (degenerate folds excluded)")
    valid = df.dropna(subset=["kappa"])
    per_subj = valid.groupby("method")["kappa"].agg(["mean", "std", "count"]).round(4)
    console.print(per_subj)

    console.rule("LOSO-on-WAUC — pooled global κ (all held-out predictions)")
    ...
    console.print()
    console.print("[bold]Within-CV reference (block-design leakage-inflated):[/bold]")
    console.print("  B1 FBCSP+sLDA   within-CV κ = 0.658 ± 0.169")
    console.print("  B2 BandPower    within-CV κ = 0.644 ± 0.189")
    console.print("  CCB             within-CV κ = 0.426 ± 0.207")
    console.print("[dim]LOSO κ vs within-CV κ quantifies how much of the within-CV score was session leakage.[/dim]")
